IF object_ID(N'[dbo].[sp_MDD_Voice_Libro_Voz_TLT_3G]') IS NOT NULL
              DROP PROCEDURE [dbo ].[sp_MDD_Voice_Libro_Voz_TLT_3G]
GO
CREATE PROCEDURE [dbo].[sp_MDD_Voice_Libro_Voz_TLT_3G] (
...
--declare @provincia as varchar(256) = 'Madrid'
--declare @simOperator as int = 1
--declare @tech as varchar(256) = 'UMTS'
...
--and v.is_CSFB=0 and (v.technology <> 'LTE' and v.technology_BSide <> 'LTE') and v.is_VOLTE = 0
...
------------------------------------------------------------------------------------
------------------------------- GENERAL SELECT
----------------- 3G Sheet for KPI Voice Aggregated Info Book
------------------------------------------------------------------------------------
...
--null As Started_4G,
--sum(v.is_VoLTE) as Started_VoLTE,
...
--null as GSM_calls_After_CSFB,
--null as UMTS_calls_After_CSFB,
--sum(v.is_SRVCC) as SRVCC
...
AVG(CAST(case when v.callDir='MO' and v.callstatus = 'Completed' then 1.0*v.cst_till_alerting end as Float))/1000.0 as CST_MO_Alerting_AVG,
AVG(CAST(case when v.callDir='MT' and v.callstatus = 'Completed' then 1.0*v.cst_till_alerting end as Float))/1000.0 as CST_MT_Alerting_AVG,
...
AVG(CAST(case when v.callDir='MO' and v.callstatus = 'Completed' then 1.0*v.cst_till_connAck end as Float))/1000.0 as CST_MO_Connect_AVG,
AVG(CAST(case when v.callDir='MT' and v.callstatus = 'Completed' then 1.0*v.cst_till_connAck end as Float))/1000.0 as CST_MT_Connect_AVG,
