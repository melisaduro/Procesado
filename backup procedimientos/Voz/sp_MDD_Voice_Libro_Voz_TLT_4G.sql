IF object_ID(N'[dbo].[sp_MDD_Voice_Libro_Voz_TLT_4G]') IS NOT NULL
              DROP PROCEDURE [dbo ].[sp_MDD_Voice_Libro_Voz_TLT_4G]
GO
CREATE PROCEDURE [dbo].[sp_MDD_Voice_Libro_Voz_TLT_4G] (
...
--declare @tech as varchar(256) = 'LTE'
...
-- GLOBAL FILTER:
-------------------------------------------------------------------------------		  
select v.sessionid, v.technology, v.is_VoLTE, v.is_SRVCC
into #All_Tests
...
Where s.sessionid=v.sessionid
 ...
 --			 or
	...
	--     )
--and v.technology like @tech+'%'
--and v.is_CSFB=1 
	  and (
			(@type='M2M' and (v.is_csfb=2 or (v.is_VOLTE in (1,2) and v.is_CSFB in (0,1)))) 
		 or (@type='M2F' and (v.is_csfb>0 or v.is_VOLTE>0)) 
		 or (v.callstatus='Failed' and @type='M2M' and (((v.is_csfb in (0,1) and v.is_volte in (0,1)) and ((v.csfb_device='A' and v.technology_Bside='LTE') or (v.csfb_device='B' and v.technology='LTE'))) or (v.is_csfb=0 and v.is_volte=0 and v.technology_Bside='LTE' and v.technology='LTE')))
		 or (v.callstatus='Failed' and @type='M2F' and (v.is_csfb=0 and v.is_VOLTE=0) and v.technology='LTE')	 
		 )
...
------------------------------------------------------------------------------------
------------------------------- GENERAL SELECT
----------------- 4G Sheet for KPI Voice Aggregated Info Book
------------------------------------------------------------------------------------

if @type = 'M2M' -- BenchMarker (Sólo las muestras con GPS)
begin
...
--sum(case when v.callstatus='Completed' then v.is_VoLTE else 0 end) as Started_VoLTE,
...
SUM(case when (v.Technology = 'LTE' or v.Technology_Bside = 'LTE' or (v.is_csfb>=0 and v.is_VoLTE>=0)) and v.callstatus in ('Completed','Dropped') then v.is_csfb+v.is_VOLTE else 0 end) As Started_4G,
...
else -- Type: M2F QPs (Todos los tests Con GPS o no)
begin
...
--***************************************************************************************************************************
--null As Started_Ended_3G,
--null As Started_Ended_2G,
--null As Calls_Mixed,
--SUM(case when (v.Technology = 'LTE' or v.is_csfb>0) and v.callstatus='Completed' then 1 else 0 end) As Started_4G,
----SUM(case when v.startTechnology like '%UMTS%' and v.endTechnology like '%UMTS%' then v.callDuration else 0 end) as Duration_3G,
----SUM(case when v.startTechnology like '%GSM%' and v.endTechnology like '%GSM%' then v.callDuration else 0 end) as Duration_2G,
