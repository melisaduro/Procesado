IF object_ID(N'[dbo].[sp_MDD_Voice_Libro_End_Calls]') IS NOT NULL
              DROP PROCEDURE [dbo ].[sp_MDD_Voice_Libro_End_Calls]
GO
CREATE PROCEDURE [dbo].[sp_MDD_Voice_Libro_End_Calls] (
...
--declare @ciudad as varchar(256) = 'MAD_SEV'
...
------------------------------------------------------------------------------------
------------------------------- GENERAL SELECT
--------------------- End Calls Disaggregated Info Book
------------------------------------------------------------------------------------
...
select  v.Sessionid,
		...
		case 
			when v.calltype='M2M' then v.SQNS_WB
			else v.SQNS_NB 
		end as SQNS,
		v.is_CSFB as Is_CSFB_Call,
		v.cst_till_alerting/1000.0 as CST_Alerting,
		v.cst_till_connAck/1000.0 as CST_Connect,
		v.CMService_UARFCN,
		v.CMService_Band,
		v.Alerting_UARFCN,
		v.Alerting_Band,
		...
		case 
			when (v.calltype='M2M' and v.MOS_WB is not null) then v.MOS_WB
			else v.MOS_NB 
		end as Average_MOS,
